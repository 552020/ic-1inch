...
contract FusionEscrow is ReentrancyGuard, Ownable {
    ...
    // Events
    event ETHLocked(string indexed orderId, address indexed maker, uint256 amount, bytes32 hashlock);
    event ETHClaimed(string indexed orderId, address indexed resolver, uint256 amount);
    event ETHRefunded(string indexed orderId, address indexed maker, uint256 amount);
    ...
    modifier onlyAuthorizedResolver() {
        require(authorizedResolvers[msg.sender], "FusionEscrow: not authorized resolver");
        _;
    }

    modifier onlyEscrowMaker(string memory orderId) {
        require(escrows[orderId].maker == msg.sender, "FusionEscrow: not escrow maker");
        _;
    }

    modifier escrowExists(string memory orderId) {
        require(escrows[orderId].maker != address(0), "FusionEscrow: escrow does not exist");
        _;
    }
    ...
    modifier escrowNotRefunded(string memory orderId) {
        require(!escrows[orderId].refunded, "FusionEscrow: escrow already refunded");
        _;
    }
    ...
    function lockETHForSwap(
        ...
        require(escrows[orderId].maker == address(0), "FusionEscrow: order already exists");
        require(timelock > block.timestamp + MIN_LOCK_TIME, "FusionEscrow: timelock too short");
        require(timelock < block.timestamp + MAX_LOCK_TIME, "FusionEscrow: timelock too long");
    ...
    ) external onlyAuthorizedResolver escrowExists(orderId) escrowNotClaimed(orderId) escrowNotRefunded(orderId) nonReentrant {
        EscrowData storage escrow = escrows[orderId];
        
        require(escrow.timelock > block.timestamp, "FusionEscrow: timelock expired");
     ...
     * @param orderId Unique identifier for the swap order
     */
    function refundLockedETH(string memory orderId) 
        external 
        ...
        nonReentrant 
    {
        ...
        require(escrow.timelock <= block.timestamp, "FusionEscrow: timelock not expired");
     ...
     * @param orderId Unique identifier for the swap order
     */
    function getEscrowData(string memory orderId) external view returns (EscrowData memory) {
     ...
     * @param orderId Unique identifier for the swap order
     */
    function escrowExistsCheck(string memory orderId) external view returns (bool) {
