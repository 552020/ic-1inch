...
use types::{MakerTraits, Order, OrderError, OrderId, SystemStats, TakerTraits};
...
fn parse_extension_args(args: &[u8]) -> Result<ExtensionData, OrderError> {
    ...
    Ok(ExtensionData { cross_chain_data: None })
}
...
async fn fill_order_args(
    ...
    if let Some(cross_chain_data) = extension.cross_chain_data {
        return fill_cross_chain_order_internal(
            order,
            signature,
            amount,
            taker_traits,
...
#[ic_cdk::query]
fn remaining_invalidator_for_order(maker: candid::Principal, order_hash: Vec<u8>) -> u64 {
    get_remaining_amount(maker, &order_hash)
}

/// Check order invalidation status - Core 1inch LOP function
#[ic_cdk::query]
fn bit_invalidator_for_order(maker: candid::Principal, slot: u64) -> u64 {
